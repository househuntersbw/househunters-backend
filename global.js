...
// Load all requests (for received requests page)
async function loadAllRequests(containerId, filterType = null) {
    const container = document.getElementById(containerId);
    if (!container) return;
    
    container.innerHTML = '<div class="listing-card">Loading requests...</div>';
    ...
    try {
        ...
        // Filter out user's own requests for received page
        const userEmail = localStorage.getItem('userEmail');
        if (filterType === 'received') {
            requests = requests.filter(r => r.posted_by !== userEmail);
        } else if (filterType === 'sent') {
            requests = requests.filter(r => r.posted_by === userEmail);
        }
        
        if (requests.length === 0) {
            container.innerHTML = '<div class="listing-card"><p>No requests found.</p></div>';
            return;
        }
        ...
        container.innerHTML = requests.map(req => `
            <div class="listing-card" style="margin-bottom: 16px;">
                <div style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 10px;">
                    <h3 style="margin: 0 0 8px 0;">${escapeHtml(req.title)}</h3>
                    <span class="request-badge" style="background: #333; padding: 4px 12px; border-radius: 20px; font-size: 12px;">
                        ${getRequestTypeDisplay(req.type)}
                    </span>
                </div>
                <p style="margin: 8px 0; color: #ccc;">${escapeHtml(req.description)}</p>
                <div style="display: flex; gap: 16px; flex-wrap: wrap; font-size: 13px; color: #888; margin: 12px 0;">
                    <span><i class="fa-solid fa-location-dot"></i> ${escapeHtml(req.location) || 'Anywhere'}</span>
                    ${req.budget ? `<span><i class="fa-solid fa-money-bill"></i> P${parseInt(req.budget).toLocaleString()}</span>` : ''}
                    <span><i class="fa-regular fa-user"></i> ${escapeHtml(req.contact_name || req.posted_by.split('@')[0])}</span>
                    <span><i class="fa-regular fa-calendar"></i> ${new Date(req.created_at).toLocaleDateString()}</span>
                </div>
                ${req.posted_by !== localStorage.getItem('userEmail') ? `
                    <div style="margin-top: 12px;">
                        <button class="btn small" onclick="startChat('${req.posted_by}', '${escapeHtml(req.title)}')">
                            <i class="fa-solid fa-comment"></i> Chat
                        </button>
                    </div>
                ` : `
                    <div style="margin-top: 12px;">
                        <button class="btn small danger" onclick="deleteRequest(${req.id})">
                            <i class="fa-solid fa-trash"></i> Delete
                        </button>
                    </div>
                `}
        ...
        container.innerHTML = '<div class="listing-card"><p>⚠️ Error loading requests</p></div>';
    }
}
...
async function deleteRequest(requestId) {
    ...
    try {
        ...
        if (result.success) {
            ...
            // Reload the page or refresh the list
            location.reload();
        } else {
            alert('❌ ' + result.message);
        }
...
// Helper function to display request type
function getRequestTypeDisplay(type) {
    const displays = {
        'looking-for-property': '🔍 Looking for Property',
        'looking-for-agent': '🤝 Looking for Agent',
        'looking-for-buyer': '💰 Looking for Buyer',
        'looking-for-seller': '🏠 Looking for Seller',
        'looking-for-tenant': '🏢 Looking for Tenant',
        'looking-for-landlord': '🏘️ Looking for Landlord'
    };
    return displays[type] || '📝 ' + type;
}

// Make request functions global
window.postRequest = postRequest;
window.loadAllRequests = loadAllRequests;
window.deleteRequest = deleteRequest;
window.getRequestTypeDisplay = getRequestTypeDisplay;
