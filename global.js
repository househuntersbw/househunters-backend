...
// ============ DARK/LIGHT MODE GLOBAL FUNCTIONS ============
...
function applyTheme(isDark) {
    if (isDark) {
        ...
        document.querySelectorAll('.listing-card, .profile-card, .settings-card, .auth-card, .modal-content, .chat-item, .message .bubble').forEach(el => {
            if (el.classList && el.classList.contains('message')) return;
            el.style.backgroundColor = '#1a1a1a';
            el.style.borderColor = '#2e2f32';
        });
        
        document.querySelectorAll('.app-header, .bottom-nav, .sidebar, .chat-area, .chat-input, .chat-sidebar, .chat-sidebar-header').forEach(el => {
            if (el) {
