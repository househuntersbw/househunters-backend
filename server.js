...
// CREATE PROPERTY - WITH BETTER ERROR HANDLING
    ...
    propertyUpload.array('images', 4)(req, res, function(err) {
        ...
        if (req.files && req.files.length > 0) {
            req.files.forEach((file, i) => {
                console.log(`File ${i}: ${file.originalname} (${file.size} bytes)`);
            });
        }
        ...
        console.log('📝 Saving listing:', { title, price, location, type, images });
