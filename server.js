...
app.post('/api/properties', (req, res) => {
    ...
    const sql = `INSERT INTO properties 
        (title, description, price, location, type, bedrooms, bathrooms, area, posted_by, images)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
...
// GET requests by user (sent by specific user)
app.get('/api/requests/sent/:email', (req, res) => {
    const email = req.params.email;
    db.all('SELECT * FROM requests WHERE posted_by = ? ORDER BY created_at DESC', [email], (err, rows) => {
        if (err) {
...
// GET requests for user (received - not posted by user)
app.get('/api/requests/received/:email', (req, res) => {
    const email = req.params.email;
    db.all('SELECT * FROM requests WHERE posted_by != ? ORDER BY created_at DESC', [email], (err, rows) => {
        if (err) {
...
app.post('/api/requests', (req, res) => {
    const { title, description, type, budget, location, contactName, contactPhone, postedBy } = req.body;
    ...
    if (!type) return res.json({ success: false, message: 'Request type is required' });
    if (!location) return res.json({ success: false, message: 'Location is required' });
    if (!contactName) return res.json({ success: false, message: 'Contact name is required' });
    if (!postedBy) return res.json({ success: false, message: 'User email is required' });
    
    const sql = `INSERT INTO requests 
        (title, description, type, budget, location, contact_name, contact_phone, posted_by)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)`;
    ...
    db.run(sql, values, function(err) {
        ...
        console.log('✅ Request created! ID:', this.lastID);
        res.json({ 
            success: true, 
...
app.delete('/api/requests/:id', (req, res) => {
    ...
    console.log('ID:', requestId, 'Email:', email);
    
    if (!email) {
        return res.json({ success: false, message: 'Email required' });
    }
    
    db.get('SELECT posted_by FROM requests WHERE id = ?', [requestId], (err, request) => {
        if (err) {
        ...
        if (request.posted_by !== email) {
            return res.json({ success: false, message: 'Unauthorized - you can only delete your own requests' });
        }
...
app.put('/api/requests/:id', (req, res) => {
    const { email, title, description, type, budget, location, contactName, contactPhone } = req.body;
    const requestId = req.params.id;
    ...
    db.get('SELECT posted_by FROM requests WHERE id = ?', [requestId], (err, request) => {
        ...
        const sql = `UPDATE requests SET 
            title = ?, description = ?, type = ?, budget = ?, location = ?, 
            contact_name = ?, contact_phone = ?
            WHERE id = ?`;
        
        const values = [title, description, type, budget || 0, location, contactName, contactPhone || '', requestId];
