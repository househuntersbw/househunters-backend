...
app.post('/api/properties', (req, res) => {
    propertyUpload.array('images', 4)(req, res, function(err) {
        ...
        // Extract fields - try both from req.body and req.body fields
        ...
        if (!type) {
            ...
            return res.json({ success: false, message: 'Property type is required' });
        }
        if (!postedBy) {
            console.error('❌ Missing postedBy');
            return res.json({ success: false, message: 'User email is required' });
        }
        
        // Process images - store just the filename
        let images = '';
